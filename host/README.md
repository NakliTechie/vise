> **Lifecycle:** living — foundations, persistent replacement and registered initialization recovery implemented; host runtime pending
...
agreement, and reconcile registered replacement and initialization transactions.
Actual evaluation/acceptance and delivery remain unfinished.
...
legacy initialization and an unverifiable first ownership write still refuse
while retaining evidence. The isolated seed builder seals exact Git bootstrap
bytes, directories, requested index and fixed identity. Initialization retains
that capsule separately from C/O before ownership publication, then reconciles
registered preparation, Git-directory publication, installation and cleanup.
Focused tests exercise four actual controller SIGKILL boundaries with two fresh
reopens each; the wider initialization syscall matrix remains pending. Arbitrary
native Git ref-transaction debris remains a preserved refusal. Isolated builders
are retained after success or failure; bounded admission and reclamation are not
yet implemented. No phase-two completion is claimed.
