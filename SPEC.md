...
- **A metric that enforces must declare a `version_cmd`.** Without one the recorded tool version is the empty string, which compares equal to the empty string forever — so replacing the analyzer, or editing a script it calls, is invisible, and "swapping the analyzer is harness drift, never a free improvement" is not true. `enforce = "none"` needs no version command, because nothing is gated on it. A version *string* is still only a string: it identifies a release, not a binary, and an analyzer that changes behavior without changing its version defeats it. That residue belongs to the operator's environment fingerprint.
- **Metrics are evaluated only when behavior held.** Not merely ranked below behavior in the verdict — not run at all. Running an analyzer against a tree whose behavior has already changed produces a number compared against a baseline recorded under different behavior, handed to an agent that is about to revert the change it describes.
...
- **Environment is sanitized, not inherited**: `PATH`, `HOME`, the stub set (`TZ`, `LANG`/`LC_ALL`, `VISE_SEED`, `SOURCE_DATE_EPOCH=0`, `VISE=1`, `PYTHONHASHSEED=0`, `NO_COLOR=1`, `TERM=dumb`, `COLUMNS=80`, `CI=1`), `VISE_TMP` and `TMPDIR` (both the per-probe scratch under `.vise/tmp/`, wiped after each run — pinning `TMPDIR` keeps every tool's temp files inside the probe's own scratch and stops the host's temp resolution from leaking warnings into the observation) — nothing else from the caller. Per-probe `env = { … }` adds declared variables. The shell itself then sets a few of its own (`PWD`, `SHLVL`, `_`), which vise does not strip: they are a function of the invocation rather than of the caller's environment, and removing them would mean not using a shell. Non-tty stdio. Per-runtime quirks: RUNTIMES.md.
- **Isolation is behavioral**: probes are order-independent; a probe writes only to declared `files` and `$VISE_TMP`. **A probe must not change the checkout** — vise snapshots the work tree before and after every judged run and classes a violation as probe error. The snapshot covers tracked files by their diff against `HEAD`, and every file Git neither tracks nor ignores by content, because a stray a probe drops into the checkout is invisible to a tracked diff and visible to every later probe and build, which is exactly how order-dependence gets in. **Ignored paths are outside the snapshot on purpose**: a build cache is the one thing a probe is expected to write, and `.gitignore` is where the operator already declared which paths those are. **A `.gitignore` or a `.gitattributes` is not one of those paths.** They decide what the comparison reads — one what *ignored* means, the other whether a path is diffed at all and through what — so an ignored one of either is still in the snapshot: without that, a probe writes a `.gitignore` naming both its stray and itself, and leaves nothing for the scan to see. Demonstrated with an ordinary shell probe. The listing is pathspec-limited to those names, so it does not walk an ignored dependency tree; on vise's own dogfood target it costs about 24 ms. Declared `files` are excluded too — vise deletes and recreates them each run by design, and compares them separately. A stray is reported by name (at most three, then a count). The untracked digest covers permission bits, symlink targets, and modification time as well as content. Permissions and targets because a `chmod +x` on a script or a symlink pointed somewhere else changes the checkout without changing a byte of any file. Modification time because a probe that writes a stray fails its first run and then rewrites the same bytes on the next one — a content-only comparison would turn *rerun it* into a way to launder a harness error, and the tracked half of this check has always failed on a mid-run write. The cost is proportional to the untracked, unignored bytes in the checkout — 18 files and 124 KiB in vise's own dogfood target, about 40 ms — and a repository that keeps a large unignored tree should ignore it, which is what `.gitignore` is for. **An empty directory a probe leaves behind is not detected**, and neither is a fifo, socket or device: `git ls-files --others` lists regular files and symlinks and nothing else, so both are invisible to the snapshot because they are invisible to Git. `git ls-files --others` lists files, so finding one means walking the whole checkout on every probe run, and that walk cannot honour the ignored-path boundary above without a `check-ignore` call per directory. The cost is certain and the case is marginal, so the limit is stated rather than paid for; it was implemented and removed the same night.
- **The judged program can see the judge, and that is not fixable here.** Every probe runs with `VISE=1` in its environment, inside the repository being judged, where `vise.lock` and `.vise/blobs/` are committed files it can read. A program changed to return its recorded output when `VISE=1`, and its new behavior otherwise, gates green forever. It needs no write to evaluator state, so the state digest does not see it, and no probe can catch it because the probe is what it is lying to. Closing the easiest half — hiding `VISE=1`, or the blobs — would break the legitimate uses those exist for (`RUNTIMES.md` is largely about programs that must be *told* they are under a harness) and would leave the repository readable anyway. **So the honest statement is the trust model, not a mitigation: vise defends a cooperative agent against its own mistakes, and does not defend against a program written to deceive it.** What catches that is the same thing that catches an edited lockfile — a human reading the diff, and CI recomputing the hash from a trusted branch. A tool that judges a program by running it cannot also be that program's adversary.

- **Git's own state is inside the snapshot, because the snapshot trusts it.** The tracked half is a diff against `HEAD` and the untracked half obeys the ignore rules, so a probe that moves `HEAD` onto a commit of its own edits, or appends to `.git/info/exclude`, changes what *unchanged* means rather than changing the checkout. Three ordinary Git commands did it. `HEAD` — both the commit it resolves to and the raw contents of the file, because `git checkout --detach` at the commit you are already on leaves the resolved value identical and the repository on no branch — `.git/info/exclude`, `.git/info/attributes`, **the configuration Git resolves** (`git config --list`, not the repository's file), and **the excludes file Git actually resolves** are digested before and after every judged run. The resolved forms matter on their own: hashing `.git/config` catches a probe that repoints `core.excludesFile` and misses one that appends to the file already pointed at, or that sets the same key at the global level. Attributes are in there because `git diff` consults them to decide whether a path is diffed at all and through what. That last one matters on its own: digesting `.git/config` catches a probe that *repoints* `core.excludesFile` and does nothing about one that appends to the file already pointed at, which usually lives outside the checkout and is obeyed exactly like `info/exclude`.

- **Containment ends at the session boundary, and that limit is stated, not implied.** A probe runs in its own process group; vise kills the group on timeout, kills it again once the shell exits (so an ordinary background child, double fork included, does not outlive the run), bounds the wait on its output pipes (1 s), refuses any run that changed `vise.toml`, `vise.lock`, or the journal or that added or removed a blob — that check wraps **every** command the manifest can name: probes, metric commands, a metric's `version_cmd`, and the `[env] fingerprint` commands, because a guard each caller has to remember is a guard some caller will forget — and compares the work tree, tracked and untracked alike, before and after every judged run — including a run that already failed, because a probe that times out or cannot be launched can still have changed the checkout on its way down, and the probes after it would inherit that — each record pass and each verify replay. What escapes all of that is a child that leaves the **session**, not merely the group: `setsid`, or a daemonize idiom that calls it. vise cannot kill such a child, and a write it makes *after* the work-tree check is not attributed to the run. Two things still hold: blob *contents* are content-addressed and re-verified whenever they are read (§5 tamper line), and the residual belongs to the probe author (start nothing you do not wait for; redirect what you background) and to the harness policy. Enforcing more would need a platform containment boundary (a pid namespace, a sandboxed runner) and is deliberately out of scope for a portable, dependency-free CLI (2026-09-02 decision).
...
- Write order, always: **blobs first** (content-addressed; orphans are harmless garbage) → **lockfile atomic rename** → **journal append**. A crash before the rename leaves the prior state plus at worst orphan blobs, pruned at the next record. A crash *after* the rename and before the journal append leaves the **new** baseline with no journal entry for it: the lockfile is the truth and the journal is a local log, so the baseline is valid and the trajectory has a gap. `record` reports that case rather than hiding it — "the baseline was written but the journal append failed" — because an operator has to know which of the two happened before the next gate keys a rerun budget to an entry that is not there.
...
A `harness` failure carries two more fields the example above has no room for.
`detail` says what broke. `operator` is `true` when the repair is in a file the
agent contract forbids an agent from writing, and it is what routes the outcome
to `next.action: human` instead of `fix_probe` — the distinction §0.3 argues for
in prose. An agent branches on that field, never on the message. Failure keys
...
vise itself enforces **`rerun` at most once**: the third consecutive `gate` or `verify` at the same commit and lock is refused (journal-checked, exit 2) with `next: human`, once two journaled flakes touch the set of checks that run. **Two flakes, not two flakes of one probe** — two different probes flaking once each exhausts a full-suite budget, because the run is what is not converging. Each of them individually still has budget, so `verify --probe a` on a probe that flaked once is still allowed, which is what an agent narrowing to diagnose actually needs. **The budget follows the probe, not the set it ran in.** It was keyed to the exact probe set, which gave every subset its own two reruns — a flake seen in the full suite did not count against `verify --probe p`, and with N probes there were as many independent budgets as there are subsets. An agent narrowing to one probe to diagnose it walked into a fresh budget without meaning to, so the accidental case was as easy as the deliberate one. The refusal is not journaled and does not reset the chain; a chain ends only at a `record`, a journaled green or red verdict (every judged `verify` or `gate` is journaled) whose probe set covers this one, or a new commit or lock — so a refusal persists until the operator commits, re-records, or changes the manifest. **Committing is a sanctioned reset and is meant to be**, because a commit is how a stuck loop makes progress and the agent contract asks for one per green step — but it follows that an agent which commits something irrelevant gets a fresh budget. That is a deliberate trade, not an oversight: the alternative is a budget an agent cannot escape even after doing real work, and vise would rather be escapable by an agent that is committing than obstructive to one that is working. **And an escape it cannot close:** a probe that leaves a *detached* child — one that starts a new session, which the process-group kill cannot reach — can have that child delete the journal after every check has passed, and the next run then sees no history and a fresh budget. That is the same session-boundary limit as §2.2, and it defeats the evaluator-state guard and Git's own state by the same route. Closing it needs a platform containment boundary, which is out of scope for v0; what stands in its place is the harness policy (deny the agent writes to `.vise/`) and the probe-author rule (start nothing you do not wait for). A journal tail that is truncated without reaching such a boundary is refused too.

### 4.2 Exit-expectation table

`record` freezes whatever exit the probe produces — **nonzero is a legitimate expectation** (a probe may assert a failure mode). "Probe error" means the probe did not produce a judgeable observation: a launch-failure condition (spawn error / 127), a timeout, a change to the checkout the probe was not entitled to make (a tracked file, or a file Git neither tracks nor ignores — §2.2), a write to evaluator state, a declared artifact that was not produced or is not a regular file, a process left holding the output pipes, a metric that did not print exactly one finite number, or self-test divergence. Exit 127 retains this classification even when an executable deliberately returns it. A captured not-found line may name a useful path or word, but it is untrusted stderr, not proof of a missing installation or shell provenance. The list is longer than "the command failed" on purpose: each of these is a run whose result cannot be compared, which is a different thing from a run that compared badly. At verify:
...
**`vise doctor`** — the operator's readiness check, run before the repository is handed to an agent. Ten static checks, each derived from a failure observed while running real coding agents against a gated checkout: no `[env] fingerprint` (a toolchain that moves without saying so); a probe `run`, `deps`, or `env` value naming an absolute path, `~`, or `$HOME` (the operator's machine is not the agent's sandbox, and the `[env] fingerprint` command is checked too because that is where an SDK or module-cache path ends up); a harness wrapper a probe runs but does not declare in `deps` (the run string is hashed, the script it calls is not, so editing the wrapper changes what is observed with no harness drift) — detected only on a file that refers to `$VISE_TMP`, because declaring the *code under test* as a dep would make every refactor of it a harness error, and a check that cannot tell harness from subject must not guess; an uncommitted `vise.lock` or blob directory (a fresh clone, which is where the agent and CI both work, cannot gate); vise's per-checkout state not ignored by Git (every gate then dirties the tree, which the agent reports as a change it did not make); no `AGENTS.md` (an agent with no written rules discovers them by breaking them); a declared artifact that Git tracks, which makes every gate a harness error because vise deletes artifacts before each run and refuses to delete a tracked file — the trap is an ordinary `git add -A` after a probe has produced one; an untracked, unignored set large enough that hashing it around every probe run makes the gate slow for no reason the operator can see; a pin's spec that is missing, unreadable, a symlink, or not the bytes `HEAD` holds (tracked is not enough — a staged file with no commit is tracked, and a fresh clone gets what `HEAD` has); and a pin's spec matched by an ignore rule, which drops out of the next commit silently. An eleventh, `manifest`, replaces all of them when `vise.toml` will not parse: there is nothing else worth saying about a repository whose manifest is broken. Each finding's `check` is a stable identifier (`env-fingerprint`, `portable-paths`, `declared-inputs`, `baseline-committed`, `local-state-ignored`, `agent-contract`, `tracked-artifacts`, `snapshot-cost`, `spec-committed`, `spec-ignored`, `manifest`, `git-work-tree`) so a script can act on one without reading its prose. Findings are grouped by cause, not by site, and each carries a remedy. Runs no probe and writes nothing. Outside a Git work tree it reports `git-work-tree` and still exits 0, because a command that says of itself that it always exits 0 for a valid invocation has no exceptions among them. **Always exit 0 for any valid invocation**, for the same reason `status` does: it reports a situation to an operator, it does not judge a change. A usage error — an argument it does not take — is exit 2 like any other, because that is not a report about the repository, and giving it a code would add a seventh meaning to a vocabulary whose value is that each code names exactly one next action.

**Doctor is advisory and best-effort, and this is deliberate.** It is a static predictor of failures a dynamic gate will hit, and the two cannot be made to agree in every case: a check that reads a manifest and the checkout on disk, without running a probe, cannot know everything the gate learns by running one. So doctor is held to a different, lower bar than the gate. Two rules bound what it promises. **It fails safe:** where doctor cannot answer — a git command that errors, a file it cannot read — it reports the uncertainty as a finding rather than passing the repository, so a doctor that says *ready* means *nothing I check is wrong*, not *nothing is wrong*. And **a doctor miss costs a surprise, never a wrong verdict:** the worst a false clean does is let the operator meet a gate failure they could have been warned about; the gate itself still refuses the bad state. The known limits are the ones a static token scan inherits: it flags a machine-local path by shape, so it can miss a path embedded in an unusual place (`--flag /abs` as one shell word) and can over-flag shell syntax that looks like a path (an `awk` operator, a regex); and it detects an undeclared harness wrapper by the heuristic that the file names `$VISE_TMP`, which a coincidental mention can trip and an unusual wrapper can evade. These are limits of prediction, not of the gate: whatever doctor misses, the gate still catches when the probe runs. Treat a clean doctor as *this repository clears the common setup traps*, not as a guarantee the gate will pass — the cold gate, which runs the probes, is the only thing that proves that.

**`vise status`** — the single perception act. The identity of the binary answering (`tool`: version, revision, modified — JSON only, so the human rendering stays stable enough to be a probe surface), manifest validity, lockfile state (probes covered, fingerprint match, `recorded_commit` spread), static manifest-versus-lock drift (`lock.drift`, capped at 5 lines in the human view; state `baseline-drift` when the next gate would refuse without running a probe), rerun-limit state (`rerun-refused` when the next gate would be refused, §4.1), last verdict, metric trajectory, journal tail (last 5 events), recent flakes. Bounded ≤ ~30 lines / one object regardless of history. Read-only in the sense that vise writes no *state*: it takes no state lock, and a repository that has never run vise is unchanged by asking it what its situation is, because the `[env] fingerprint` commands run only once a lockfile exists. Where one does, running them needs a scratch directory, so status recreates `.vise/tmp` — in a repository that by definition already has a `.vise/`, and asking is possible *while* a record or gate holds the lock — the moment you most want to ask what is happening is while something is happening. `doctor` is read-only on the same terms. An unrecognized command is refused before the lock is reached, so a typo cannot wait out a running suite. It does execute the manifest's `[env] fingerprint` commands to report live environment drift — they are operator-declared like probes, and a stale drift report would send the agent into a refusing gate; a fingerprint command that writes files is an operator bug, caught by the same work-tree snapshot probes get. **Always exit 0 for any valid invocation** (the one deliberate exception to the shared vocabulary — status *reports* red, it doesn't *fail* red; stated here so it is contract, not accident). A usage error is exit 2: it is a complaint about the command line, not a report about the repository.

**`vise record`** — freeze. Two full suite passes (§2.2), byte-identical lockfiles required, then blobs → lockfile → journal (§3.1). Requires a clean working tree (frozen truth must correspond to a returnable commit); `--allow-dirty` overrides and journals `"dirty": true`. **Overwriting an existing lockfile** requires a review gesture, and the review has to be one a reader could actually perform. A candidate whose diff cannot show what changed is refused rather than offered: a recorded observation whose blob is missing or fails its content hash (ordinary blob loss — an incomplete checkout is enough), and an observation over the capture bound, where the reader is shown two hashes and a one-byte change is indistinguishable from a rewrite. The remedy is named: the blobs are committed, so restore them; if the old baseline is genuinely gone, delete `vise.lock` and record afresh, which says out loud that nothing was compared. Two-phase, agent-safe: `record --preview` runs both passes and returns the candidate's behavior diff old-vs-new (§5) plus `candidate`, the sha256 of the canonical candidate lockfile, writing no baseline state (no blobs, no lock, no journal event — the probes still run and regenerate their declared artifacts); `record --accept <digest>` runs the passes again and writes only if the candidate still matches — a tree or environment change in between is refused. One-step, human-shaped: `--i-reviewed-the-diff` prints the diff and writes. All of these compose with `--allow-dirty`; all are operator-territory. Exit: 0 · 2 (probe error, dirty tree, no git) · 3 (self-test divergence).
