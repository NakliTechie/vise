...
`record` freezes whatever exit the probe produces — **nonzero is a legitimate expectation** (a probe may assert a failure mode). "Probe error" means the probe did not produce a judgeable observation: a launch failure (spawn error / 127, with the missing word named), a timeout, a change to the checkout the probe was not entitled to make (a tracked file, or a file Git neither tracks nor ignores — §2.2), a write to evaluator state, a declared artifact that was not produced or is not a regular file, a process left holding the output pipes, a metric that did not print exactly one finite number, or self-test divergence. The list is longer than "the command failed" on purpose: each of these is a run whose result cannot be compared, which is a different thing from a run that compared badly. At verify:
